...
namespace FormInteg\ZCFORMINATOR\Flow;

use FormInteg\ZCFORMINATOR\Core\Util\Common;
use FormInteg\ZCFORMINATOR\Core\Util\IpTool;
use FormInteg\ZCFORMINATOR\Core\Util\SmartTags;
use FormInteg\ZCFORMINATOR\Log\LogHandler;
use FormInteg\ZCFORMINATOR\Triggers\TriggerController;
use WP_Error;
...
final class Flow
{
    ...
    public function get($data)
    {
        ...
        if (!\is_null($missingField)) {
            wp_send_json_error(sprintf(__('%s can\'t be empty', 'zc-forminator'), $missingField));
        }
        $integrationHandler = new FlowController();
        $integrations       = $integrationHandler->get(
            ['id' => $data->id],
    ...
    public function save($data)
    {
        ...
        if (!\is_null($missing_field)) {
            wp_send_json_error(sprintf(__('%s can\'t be empty', 'zc-forminator'), $missing_field));
        }
        ...
        wp_send_json_success(['id' => $saveStatus, 'msg' => __('Integration saved successfully', 'zc-forminator')]);
    }
    ...
    public function flowClone($data)
    {
        ...
        if (!\is_null($missingId)) {
            wp_send_json_error(sprintf(__('%s can\'t be empty', 'zc-forminator'), $missingId));
        }
        $integrationHandler = new FlowController();
        $integrations       = $integrationHandler->get(
            ['id' => $data->id],
        ...
        if (!is_wp_error($integrations) && \count($integrations) > 0) {
            ...
            wp_send_json_error(__('Flow ID is not exists', 'zc-forminator'));
        }
    }
    ...
    public function update($data)
    {
        ...
        wp_send_json_success(__('Integration updated successfully', 'zc-forminator'));
    }
    ...
    public function delete($data)
    {
        ...
        if (!\is_null($missing_field)) {
            wp_send_json_error(sprintf(__('%s cann\'t be empty', 'zc-forminator'), $missing_field));
        }
        $integrationHandler = new FlowController();
        ...
        wp_send_json_success(__('Integration deleted successfully', 'zc-forminator'));
    }

    public function bulkDelete($param)
    {
        if (!\is_array($param->flowID) || $param->flowID === []) {
            wp_send_json_error(sprintf(__('%s cann\'t be empty', 'zc-forminator'), 'Integration id'));
        }
    ...
    public function toggle_status($data)
    {
        ...
        wp_send_json_success(__('Status changed successfully', 'zc-forminator'));
    }
        ...
        if (!is_wp_error($flows) && !empty($flows)) {
            ...
            foreach ($flows as $flowData) {
                ...
                if (
                    ...
                    $error = new WP_Error('Conditional Logic False', __('Conditional Logic not matched', 'zc-forminator'));
    ...
    {
        if (class_exists("FormInteg\\ZCFORMINATOR\\Actions\\{$name}\\{$name}Controller")) {
            return "FormInteg\\ZCFORMINATOR\\Actions\\{$name}\\{$name}Controller";
        }
    ...
    {
        if (class_exists("FormInteg\\ZCFORMINATOR\\Triggers\\{$name}\\{$name}Controller")) {
            return "FormInteg\\ZCFORMINATOR\\Triggers\\{$name}\\{$name}Controller";
        }
