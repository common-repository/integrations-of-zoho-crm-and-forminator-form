...
namespace FormInteg\ZCFORMINATOR\Actions\ZohoCRM;

use FormInteg\ZCFORMINATOR\Core\Util\DateTimeHelper;
use FormInteg\ZCFORMINATOR\Core\Util\HttpHelper;
    ...
    public function execute($integId, $defaultConf, $module, $layout, $fieldValues, $fieldMap, $actions, $required, $fileMap = [], $isRelated = false)
    {
        ...
        if (!empty($actions->gclid)) {
            ...
            if (isset($fieldValues['gclid'])) {
                ...
                $gclid = preg_replace('/[^a-zA-Z0-9\-\.]/', '', $_REQUEST['zc_gad']);
            }
        ...
        if (
            ...
            if (!empty($actions->tag_rec) && class_exists('FormInteg\ZCFORMINATOR\Actions\ZohoCRM\TagApiHelper')) {
                $tags    = '';
            ...
            if ($apiFormat === 'array' || $apiFormat === 'object') {
                ...
                if (\is_array($value)) {
                    if (isset($value['date'])) {
                        ...
                        $value       = $value['time'];
                        $date_format = 'H:i A';
                    } elseif (isset($value['time'], $value['date'])) {
